...
function StatusCard({ label, value, status }: { label: string; value: string | number; status?: string }) {
  const cls =
  ...
  return (
    <Panel title="CRK‑1 Kernel Monitor">
      <div className={styles.grid}>
        <StatusCard label="Invariant Engine" value={`${status.activeInvariants} active`} status={status.invariantEngine} />
        <StatusCard label="Pattern Ledger" value={`${status.receiptCount} receipts`} status={status.ledger} />
        <StatusCard label="Continuity Substrate" value={`${status.snapshotCount} snapshots`} status={status.continuity} />
        <StatusCard label="Violations (1m)" value={status.violationsLastMinute} />
      </div>
      <div className={styles.summary}>
        Replay Consistency: <span className={nominal ? styles.pass : styles.check}>{nominal ? "PASS" : "CHECK"}</span>
        <br />
