...
import { authorizeReceiptIngest } from "./ingest-auth";
...
const router: Record<string, Record<string, (req: IncomingMessage, res: ServerResponse) => Promise<void>>> = {
  ...
  "/api/receipts/ingest": {
    POST: async (req, res) => {
      if (!authorizeReceiptIngest(req, res, (status, message) => error(res, status, message))) {
        return;
      }
      try {
...
const server = createServer(async (req, res) => {
  if (req.method === "OPTIONS") {
    res.writeHead(204, {
      ...
      "Access-Control-Allow-Headers": "Content-Type, X-Nova-Ingest-Token",
    });
    return res.end();
  }
...
bootNovaSpine()
  .then(() => {
    server.listen(PORT, () => {
      ...
      console.log(
        "Active: AgentRuntime · CRK-2 · Sovereign X · Control Tower · Fabric · SSE (receipts · ISL · amendments · delegation · stewardship)",
      );
    });
  })
  .catch((err) => {
