...
describe("CRVS evidence refresh + anti-fabrication", () => {
  ...
  it("CenterCommand source does not ship fabricated default checklist labels", async () => {
    const { readFileSync } = await import("node:fs");
    const src = readFileSync(new URL("../cockpit/src/hud/CenterCommand.tsx", import.meta.url), "utf8");
    assert.ok(!src.includes("Understand Request"));
    assert.ok(!src.includes("Bind Evidence"));
    assert.ok(!src.includes("Await Authority"));
    assert.ok(!src.includes("Execute & Verify"));
    assert.ok(!src.includes("authOk ? 94 : 62"));
  });
});
