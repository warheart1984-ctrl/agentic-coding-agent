...
import { upsertReceipt } from "./receiptMerge";
...
export const useCockpitState = create<CockpitState>((set) => ({
  ...
  actions: {
    ...
    addReceipt: (r) =>
      set((s) => {
        const next = upsertReceipt(s.governance, s.continuity, r);
        return {
          governance: { ...s.governance, ...next.governance },
          continuity: { ...s.continuity, ...next.continuity },
        };
      }),
    addViolation: (v) =>
      set((s) => ({
    ...
    addReceiptFromGateway: (r) =>
      set((s) => {
        if (s.governance.receipts.some((x) => x.id === r.receiptId)) {
          return s;
        }
        return {
          governance: {
